...
export default function AuthGate({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function verifyStoredSession() {
      ...
      // Browser storage alone does not authorize Headquarters. Confirm the
      // stored token with Supabase Auth and use the returned app_metadata.
    ...
    const { data: subscription } = supabase.auth.onAuthStateChange((event, nextSession) => {
      ...
      // INITIAL_SESSION may come only from restored browser storage. The
      // server-backed verification above exclusively owns initial access.
      ...
      <div className="min-h-screen grid place-items-center px-6">
        <form onSubmit={signIn} className="w-full max-w-sm border border-ink-700 bg-ink-900/90 p-6 space-y-4">
          <div>
            ...
            <p className="mt-2 text-sm text-ink-400">HQ data and controls require an authenticated owner or operator account.</p>
          </div>
